...
void *TrainModelThread(void *id) {
  ...
  while (1) {   
    ...
    int cw = 0; // count the actual num of context words
    if(join_type == 1){
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        for (c = 0; c < vocab[last_word].character_size; c++) {
          ...
          for (d = 0; d < char2comp[char_id].comp_size; d++){
            comp_id = char2comp[char_id].comp[d];
            comp_id_list[comp_list_cnt++] = comp_id;
            for (e = 0; e < layer1_size; e++) neucomp[e] += syncomp[e + comp_id * layer1_size] / char2comp[char_id].comp_size / vocab[last_word].character_size;
          }
        }
        cw++;
      }
